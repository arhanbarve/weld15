...
export function CameraRig() {
   ...
   *   stages 1, 2, 4 no drag; wheel scrubs the journey.
   *   stage 3        drag orbits (writes `orbit`); wheel changes the orbit radius. UNCHANGED.
  ...
  useEffect(() => {
    ...
    const currentOrbit = () => useStore.getState().orbit ?? orbitOf(base);
    ...
    const onDown = (e: PointerEvent) => {
      ...
      // Only stages 0 and 3 have anything for a drag to do; 1, 2 and 4 are fixed shots
      // that scrub on the wheel alone, same as before this step.
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      // Stages 0, 1, 2 and 4: the wheel scrubs the whole journey instead, in the same
