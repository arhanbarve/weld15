 ...
 *
 * DRAW CALLS: four meshes here, five while the sweep runs. walls, roof, towers and
 * bays are the four geometries buildWeld() returns; the towers share the roof's
 * material, and <Threshold> merges walls and roof into the one extra mesh. Measured
 * at stage 4 with reduced motion, where nothing else is on screen and the composer is
 * off: 4 calls, 416 triangles, which is 220 for the extruded 56-point ring, 112 for
 * the gable's 56 fan quads, 24 for two roof features and 60 for five window bays.
 * Against a budget of 8 that leaves three spare, and the reason not to spend them on
 * a material feature is the one materials() records against `transmission` -- it
...
import { buildWeld, TOWER_DEFAULTS, type TowerParams } from "./weldGeometry";
...
export function WeldExterior({
  ...
  const geo = useMemo(() => buildWeld(suiteParams, towers), [suiteParams, towers]);
  ...
  return (
    <group visible={visible}>
      <mesh geometry={geo.walls} material={pal.walls} />
      <mesh geometry={geo.roof} material={pal.roof} />
      <mesh geometry={geo.towers} material={pal.roof} />
      <mesh geometry={geo.bays} material={pal.bays} />
