...
- **Ambient occlusion.** Add GTAO to the existing `EffectComposer`. This is what makes a baseboard
  read as a baseboard and a drawer front read as recessed. Gated off under reduced motion alongside
  bloom, and gated off below a measured frame-time budget.
...
| **A4** | env map, GTAO, plaster normal, hardware material | `materials.test.ts`; screenshots; frame-time measurement |
